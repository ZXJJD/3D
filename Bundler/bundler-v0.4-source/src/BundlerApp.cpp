...
bool BundlerApp::OnInit()
{
	...
	//argc存储的是命令行（输入的参数+1）个数
	/*
	eg：命令行输入 a1、a2、a3，argc等于4；
	而argv存储的是所有输入的参数，并且默认argv[0]是需要执行的exe文件位置
	*/
	//而这里argc应该为2，存储的是：XX.exe list.txt
	//而list里面存储的是图片的访问路径
    if (argc >= 2) {
	printf("Loading images from file '%s'\n", argv[1]);
	imageList = argv[1];
	load_file = true;//表示对于（图像）文件的获取路径已经加载成功
    } else {
	PrintUsage();//显示一些相关使用信息而已
	exit(0);
    }

    printf("[BundlerApp::OnInit] Processing options...\n");
	//将命令行输入的参数进行计算，出去exe的路径，保存输入的参数
	...
	//强行的将缓存区的所以数据，转存傲stdout数据流上
    fflush(stdout);
    if (load_file) {//已经获取到文件路径
