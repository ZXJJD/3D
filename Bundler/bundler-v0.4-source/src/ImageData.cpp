...
void ImageData::InitFromString(char *buf, char *path, bool fisheye_by_default) 
{
    ...
    std::string str(buf);//将字节流转化为string进行讨论
    std::vector<std::string> toks;
    Tokenize(str, toks, " ");//toks存储了输入str中的所有文件相关内容（即需要的内容）
	...
	/*../examples/kermit/kermit000.jpg 0 660.80306*/
    ...
    if (*name_end == 0) {
        ...
        m_user_name[name_end - base_name] = 0;//以0进行文件名的结尾

        strcpy(m_flickr_index, name_end + 1);//文件后缀名称
    ...
    char key_buf[256];
	//对输入的文件名进行加工，将文件的后缀该问.key，该文件用来存储之前sift获取到的特征点记录文件
...
void ImageData::GetBaseName(char *buf) {
    int len = strlen(m_name);//输入的路径长度
    
    int i;
    for (i = len - 1; i >= 0 && m_name[i] != '/'; i--) { }//从后向前，获取基本文件名称

    if (i == -1)
	strcpy(buf, m_name);
    else
	strcpy(buf, m_name + i + 1);
    //********************************************不知道为什么，从表面上是将.key的.换为0
    buf[strlen(buf) - 4] = 0;
}
